...
# Define states
START_ROUTES, RECOMMEND_STATE = range(2)


# Function triggered by the /recommend command in the START_ROUTES state
async def recommendInterest(update: Update, context: CallbackContext) -> int:
    await update.message.reply_text(
        """
            Please enter a field/interest you want to learn more about
        """
    )
    return RECOMMEND_STATE


async def handle_interest_input(update: Update, context: CallbackContext) -> int:
    ...
    asyncio.create_task(
        process_recommendation(update, context, user_input, wait_message)
    )
...
async def process_recommendation(
    update: Update, context: CallbackContext, user_input: str, wait_message
):
    try:
        # Call the asynchronous recommendation function
        chatgpt_response = await get_chatgpt_recommendation(update, user_input)

        # Delete the "Please wait" message once the recommendation is ready
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "Enter another Job Description!", callback_data="jobdescription"
                )
            ],
            [
                InlineKeyboardButton(
                    "Enter a field you want to learn more about!",
...
def format_mentor_recommendation(mentor_recommendation: dict) -> str:
    ...
    for i, mentor in enumerate(list_of_mentors):
        ...
        specialized_field = mentor.get(
            "specialized_field", "Specialization not available"
        )
        experience_years = mentor.get("experience_years", "Experience not available")
        email = mentor.get("email", "Email not available")
        reason_for_recommendation = mentor.get(
            "reason_for_recommendation", "Reason not provided"
        )
